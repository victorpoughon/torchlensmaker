...
def demo_reflection(theta_i: float, normal_angle: float) -> None:
    ...
    reflected = tlm.reflection(V, normal)
...
![png](reflection_and_refraction_files/reflection_and_refraction_2_0.png)
...
from torchlensmaker.physics.physics import refraction
...
def demo_batched_refraction(crit_option, theta_i: float, normal_angle: float, n1, n2):
    ...
    refracted, _ = refraction(V, all_normal, torch.tensor(n1), torch.tensor(n2), critical_angle=crit_option)
...
![png](reflection_and_refraction_files/reflection_and_refraction_4_1.png)
...
![png](reflection_and_refraction_files/reflection_and_refraction_4_4.png)
...
![png](reflection_and_refraction_files/reflection_and_refraction_4_7.png)
...
def demo_light(a, b, alpha_n, beta_n, function):
    ...
    # verify unit norm
    assert torch.all(
        torch.le(torch.abs(torch.linalg.norm(outcident, dim=1) - 1.0), 1e-5)
    )

    surface = tlm.Square(2.0)
...
def refract(n1, n2, critical_angle="drop"):
    def f(rays, normals):
        return tlm.refraction(
            rays,
            normals,
            n1=torch.tensor(n1),
            n2=torch.tensor(n2),
...
<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_0.json?url" />



<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_1.json?url" />



<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_2.json?url" />
...
demo_light(deg(-30), deg(-50), 1, 1, refract(n1=1.0, n2=1.5))
```


<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_3.json?url" />



```python
# a 2D cone of 3D rays entering glass
demo_light(deg(-30), deg(-50), 1, 30, refract( n1=1.0, n2=1.5))
```


<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_4.json?url" />



```python
# a 3D cone of rays entering diamond
demo_light(deg(-30), deg(-50), 30, 30, refract( n1=1.0, n2=2.417))
```


<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_5.json?url" />
...
demo_light(deg(-30), deg(-20), 30, 30, refract(n1=1.5, n2=1.0, critical_angle="drop"))
```


<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_6.json?url" />
...
demo_light(deg(-60), deg(-20), 30, 20, refract(n1=2.417, n2=1.0, critical_angle="reflect"))
```


<TLMViewer src="./reflection_and_refraction_files/reflection_and_refraction_7.json?url" />
