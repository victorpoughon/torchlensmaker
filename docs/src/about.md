---
layout: doc
sidebar: false
---

<script setup>
import Newsletter from './components/Newsletter.vue';
</script>

# About

## Funding

This project is funded through [NGI0 Commons
Fund](https://nlnet.nl/commonsfund), a fund established by
[NLnet](https://nlnet.nl) with financial support from the European Commission's
[Next Generation Internet](https://ngi.eu) program. Learn more at the [NLnet
project page](https://nlnet.nl/project/Torch-LensMaker).

[<img src="https://nlnet.nl/logo/banner.png" alt="NLnet foundation logo" width="40%" />](https://nlnet.nl)
[<img src="https://nlnet.nl/image/logos/NGI0_tag.svg" alt="NGI Zero Logo" width="40%" />](https://nlnet.nl/commonsfund)

## About me

Hello! My name is Victor, I'm the main author of this project. You can find
contact info on [my personal website](https://victorpoughon.fr). Feel free to
reach out, I love getting emails written by humans :)

## Sponsoring

I've been working on this project since 2024. First on my own time and money for
a few months, and now with support from [NLnet](https://nlnet.nl) for 2026. I'd
love to keep working on it as much as possible! If you want to contribute to
open-source optical modeling, please get in touch or make a donation! Thank
you 💚

* [Sponsor Torch Lens Maker on GitHub](https://github.com/sponsors/victorpoughon)
* [Buy me a coffee](https://buymeacoffee.com/victorpoughon)

## Newsletter

If you want to receive project updates by email, I've setup a mailing list. I'll
send a few emails per year at most.

<Newsletter/>
