...
class Parabola(SurfaceElement):
    ...
    def __init__(
        ...
        *,
        ...
        self.func2d = SagSurfaceKernel(2, ti.parabolic_sag_2d, self.solver_config)
        self.func3d = SagSurfaceKernel(3, ti.parabolic_sag_3d, self.solver_config)
        self.kernel_outer_extent = SagOuterExtentSurfaceKernel(ti.parabolic_sag_2d)
    ...
    def outer_extent(self, anchor: ScalarTensor) -> ScalarTensor:
        return self.kernel_outer_extent.apply(
            anchor, self.diameter, self.normalize, self.A.unsqueeze(0)
        )
